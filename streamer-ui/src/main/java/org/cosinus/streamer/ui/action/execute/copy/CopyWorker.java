...
public class CopyWorker<
    S extends ContainerStreamer<? extends Streamer>,
    T extends ContainerStreamer<? extends Streamer>> extends SwingProgressWorker<CopyProgressModel> {
    ...
    protected void copyBinaryStreamer(BinaryStreamer source, BinaryStreamer target) {
        boolean append = copyModel.shouldAppendSourceToCurrentTarget() || copyModel.shouldResumeCopy();
        try (BinaryStream sourceStream = source.stream();
             StreamPipeline targetPipeline = target.pipeline(append)) {

            if (copyModel.shouldResumeCopy() && !resume(sourceStream, source, target)) {
                return;
            }

            targetPipeline.consume(sourceStream, bytes -> {
                checkActionStatus();
                progress.updateStreamerProgress(bytes.length);
                publishProgress(progress);
            });

            if (sourceStream.checksum() != targetPipeline.checksum()) {
